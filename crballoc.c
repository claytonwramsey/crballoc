...
/**
 * A red-black tree memory allocator implementation, designed for use in Linux environments.
 *
...
/**
 * The error code returned by `sbrk` on error.
 */
...
void *malloc(size_t size) {
    ...
    if (best_node == NULL) {
        if (last_block == NULL || is_allocated(last_block->size_flags)) {
            ...
            BlockHeader *const bh = sbrk(BLOCK_OVERHEAD + asize);
            if (bh == SBRK_ERROR)
                return NULL;
            ...
            if (sbrk(asize - data_size(last_block->size_flags)) == SBRK_ERROR)
                return NULL;
    ...
    DEBUG_ASSERT((char *)to_return + size < (char *)sbrk(0));
    DEBUG_EXECUTE(check_heap());
    return to_return;
}
...
void *realloc(void *ptr, size_t size) {
    ...
    if (bh == last_block) {
        ...
        const size_t asize = adjust_size(size);
        void *const break_ptr = sbrk(asize - old_size);
...
static void check_heap() {
    ...
    if (last_block == NULL)
        DEBUG_ASSERT(FIRST_BLOCK == sbrk(0));
    else
        DEBUG_ASSERT((char *)find_footer(last_block) + sizeof(Footer) == sbrk(0));
